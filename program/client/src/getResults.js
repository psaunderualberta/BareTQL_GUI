...
const pipe = [nlp.string.lowerCase, nlp.string.tokenize0];
...
class ResultService {
  static getKeywords(keywords) {
    ...
    return new Promise((resolve, reject) => {
      try {
        var url =
          kwURL +
          keywords.replace(new RegExp(/ *, */, "gi"), () => {
            return "&keyword=";
          });

        axios.get(url).then((response) => {
          var tables = ResultService.handleResponse(response);
          tables = ResultService.rankTables(
            tables,
            keywords.replace(/ *, */g, " ")
          );
  ...
  static swapCells(indices) {
    ...
    return new Promise((resolve, reject) => {
      try {
        var url = `${swapURL}${indices
          .map((index) => index[0])
          .join("&rowIDs=")}&colIDs=${indices
          .map((index) => index[1])
          .join("&colIDs=")}`;
        axios.get(url).then((response) => {
  ...
  static handleDotOps(dotOp, sliderValues, uniqueCols, rowsReturned) {
    ...
    return new Promise((resolve, reject) => {
      try {
        var url = `${dotOpURL}${dotOp}&sliders=${sliderValues.join(
          "&sliders="
        )}&unique=${uniqueCols.join("&unique=")}&rowsReturned=${rowsReturned}`;
        axios.get(url).then((response) => {
          resolve(response["data"]);
        });
      } catch (err) {
  ...
  static addTablesToEngine(tables, engine) {
    ...
    Object.keys(tables).forEach((table_id) => {
      ...
      curTable["rows"] = Object.values(tables[table_id]["rows"])
        .join(" ")
        .split(" || ")
        .join(" ");
