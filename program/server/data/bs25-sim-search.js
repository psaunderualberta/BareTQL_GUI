...
class BS25 {
  ...
  defineConfig(cfg) {
    ...
    if (!cfg["bs25Params"] || !cfg["terms"])
      throw Error(
        "BS25: Both parameters and search terms must defined when calling config"
      );
    if (
      typeof cfg["terms"] !== "object" ||
      cfg["terms"].some((term) => typeof term !== "string")
    )
  ...
  addDoc(doc, id) {
    ...
    if (typeof this.documents[id] !== "undefined")
      throw Error(
        `BS25: ID '${id}' is already linked to previous document. All documents must have unique IDs.`
      );
    if (typeof id !== "string" || typeof doc !== "string") {
      throw Error(
        `BS25: Both ID and Document must be strings, received ${typeof id} and ${typeof doc} respectively`
      );
    }
    ...
    for (let key of Object.keys(this.documents)) {
      ...
      for (let [i, sim] of this.documents[key]["sims"].entries()) {
        ...
        denom =
          sim +
          this.k1 *
            (1 - this.b + (this.b * this.documents[key]["len"]) / this.avgLen);
        sum += num / denom;
      }
